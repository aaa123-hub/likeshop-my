    ...
    <view class="home-page">
        ...
        <view class="home-content">
            ...
            <view v-if="recommendedProductList.length" class="goods-grid">
                <navigator v-for="(item, index) in recommendedProductList" :key="index" class="goods-card" hover-class="none" :url="'/bundle/pages/goods_details/goods_details?id=' + (item.id || item.goods_id)">
...
export default {
    ...
    methods: {
        ...
        handleBannerTap(item) {
            if (item.url) {
                if (item.type === 'switchTab') this.switchTab(item.url)
...
<style lang="scss">
.home-page { --page-safe-top: var(--status-bar-height, 44rpx); min-height: 100vh; padding-bottom: calc(40rpx + var(--window-bottom)); background: #f5f5f5; }
.home-hero { position: relative; min-height: 496rpx; padding: calc(var(--page-safe-top) + 64rpx) 24rpx 0; overflow: hidden; background: linear-gradient(180deg, #1688ff 0%, #74b2ff 100%); }
.home-copy { position: relative; z-index: 2; color: #ffffff; }
.home-hi { font-size: 52rpx; font-weight: 600; line-height: 72rpx; }
.home-title { margin-top: 16rpx; font-size: 40rpx; font-weight: 500; line-height: 56rpx; }
.home-hero__image { position: absolute; right: -18rpx; top: calc(var(--page-safe-top) + 44rpx); width: 456rpx; height: 366rpx; z-index: 1; }
.home-search { position: absolute; left: 24rpx; right: 24rpx; bottom: 30rpx; z-index: 3; display: flex; align-items: center; justify-content: space-between; height: 82rpx; padding: 0 30rpx 0 40rpx; background: #ffffff; border-radius: 42rpx; box-sizing: border-box; }
.home-search__placeholder { color: #b3b3b3; font-size: 28rpx; }
.home-search__icon { position: relative; width: 44rpx; height: 44rpx; color: #222222; }
.home-search__icon-circle { position: absolute; left: 7rpx; top: 6rpx; width: 23rpx; height: 23rpx; border: 4rpx solid currentColor; border-radius: 50%; box-sizing: border-box; }
...
.promoter-card__action { flex: none; padding: 0 20rpx; color: #ffffff; font-size: 24rpx; line-height: 54rpx; border-radius: 28rpx; background: #ff9f1c; }
.banner-wrap { margin-top: 22rpx; }
.banner-swiper { height: 220rpx; border-radius: 18rpx; overflow: hidden; background: #f1f2f5; }
.banner-image { width: 100%; height: 220rpx; }
.quick-strip { display: flex; align-items: center; justify-content: space-between; height: 144rpx; margin-top: 22rpx; padding: 0 22rpx; border-radius: 16rpx; background: #ffffff; box-sizing: border-box; }
.quick-item { flex: 1 1 0; min-width: 0; text-align: center; }
.quick-image { display: flex; align-items: center; justify-content: center; width: 66rpx; height: 66rpx; margin: 0 auto; border-radius: 50%; background: #edf3fb; }
.quick-more__icon { width: 52rpx; height: 52rpx; margin: 0 auto; }
.quick-name { margin-top: 12rpx; color: #222222; font-size: 24rpx; line-height: 32rpx; }
.is-blue { color: #1688ff; }
.section-title { margin: 34rpx 0 20rpx; color: #222222; font-size: 34rpx; font-weight: 600; line-height: 48rpx; }
.recent-visit-panel { margin-top: 28rpx; padding: 22rpx 0 24rpx; border-radius: 18rpx; background: #ffffff; overflow: hidden; }
.recent-visit-head { display: flex; align-items: center; justify-content: space-between; padding: 0 22rpx; }
.recent-visit-title { color: #222222; font-size: 32rpx; font-weight: 600; line-height: 44rpx; }
.recent-visit-more { display: flex; align-items: center; gap: 8rpx; color: #666666; font-size: 24rpx; line-height: 34rpx; }
.recent-visit-more__icon { position: relative; width: 18rpx; height: 24rpx; }
.recent-visit-more__icon::after { content: ''; position: absolute; left: 2rpx; top: 5rpx; width: 10rpx; height: 10rpx; border-top: 3rpx solid #1688ff; border-right: 3rpx solid #1688ff; transform: rotate(45deg); }
...
.recent-shop-card__image { display: flex; align-items: center; justify-content: center; width: 86rpx; height: 86rpx; margin: 0 auto; border-radius: 50%; background: #eef1f5; }
.recent-shop-card__name { width: 132rpx; margin: 12rpx auto 0; color: #222222; font-size: 24rpx; line-height: 32rpx; }
.image-placeholder { color: #9ca3af; font-size: 22rpx; line-height: 28rpx; text-align: center; background: #eef1f5; }
.activity-list, .goods-grid { display: grid; grid-template-columns: 1fr 1fr; gap: 18rpx; }
.activity-card { position: relative; min-height: 180rpx; padding: 24rpx; border-radius: 18rpx; background: #ffffff; box-sizing: border-box; }
.activity-card__title { color: #222222; font-size: 30rpx; font-weight: 600; line-height: 42rpx; }
.activity-card__desc { width: 58%; margin-top: 12rpx; color: #999999; font-size: 24rpx; line-height: 34rpx; }
...
.goods-card { min-width: 0; padding: 16rpx; background: #ffffff; border-radius: 18rpx; box-sizing: border-box; }
.goods-card__image { display: flex; align-items: center; justify-content: center; width: 100%; height: 220rpx; border-radius: 14rpx; background: #f3f6fb; }
.goods-card__name { min-height: 64rpx; margin-top: 14rpx; color: #222222; font-size: 26rpx; line-height: 34rpx; }
.goods-card__desc, .goods-card__shop { margin-top: 8rpx; color: #7b8494; font-size: 22rpx; line-height: 30rpx; }
.goods-card__tags { display: flex; flex-wrap: wrap; gap: 8rpx; min-height: 34rpx; margin-top: 10rpx; overflow: hidden; }
.goods-card__tag { max-width: 132rpx; padding: 0 10rpx; color: #1688ff; font-size: 20rpx; line-height: 32rpx; border-radius: 16rpx; background: #eef6ff; box-sizing: border-box; }
.goods-card__bottom { display: flex; align-items: baseline; min-width: 0; margin-top: 10rpx; }
.goods-card__price { color: #ff2c3c; font-size: 28rpx; font-weight: 600; line-height: 38rpx; }
.goods-card__market { margin-left: 10rpx; color: #b5bac4; font-size: 22rpx; line-height: 30rpx; text-decoration: line-through; }
.goods-card__meta { display: flex; align-items: center; justify-content: space-between; min-height: 30rpx; margin-top: 6rpx; color: #9aa2af; font-size: 22rpx; line-height: 30rpx; }
.shop-list { display: flex; flex-direction: column; gap: 16rpx; }
.shop-card { display: flex; align-items: center; padding: 18rpx; background: #ffffff; border-radius: 18rpx; box-sizing: border-box; }
.shop-card__logo { flex: none; display: flex; align-items: center; justify-content: center; width: 96rpx; height: 96rpx; margin-right: 18rpx; border-radius: 14rpx; background: #f3f6fb; }
.shop-card__body { flex: 1; min-width: 0; }
.shop-card__name { color: #222222; font-size: 28rpx; font-weight: 600; line-height: 40rpx; }
.shop-card__address { margin-top: 8rpx; color: #999999; font-size: 24rpx; line-height: 34rpx; }
.shop-card__meta { margin-left: 16rpx; text-align: right; }
.shop-card__status { color: #1688ff; font-size: 24rpx; line-height: 34rpx; }
.shop-card__score { margin-top: 8rpx; color: #222222; font-size: 26rpx; font-weight: 600; line-height: 36rpx; }
.identity-title { margin: 50rpx 0 26rpx; color: #222222; font-size: 36rpx; font-weight: 600; line-height: 50rpx; }
.identity-card { display: flex; align-items: center; justify-content: space-between; height: 210rpx; margin-bottom: 24rpx; padding: 0 54rpx 0 58rpx; border-radius: 14rpx; box-sizing: border-box; }
.identity-card--seller { border: 2rpx solid #dcecff; background: linear-gradient(180deg, #edf7ff 0%, #ffffff 100%); }
.identity-card--buyer { border: 2rpx solid #fff2d4; background: linear-gradient(180deg, #fff9eb 0%, #ffffff 100%); }
.identity-card__title { color: #222222; font-size: 38rpx; font-weight: 600; line-height: 52rpx; }
.identity-card__desc { margin-top: 14rpx; color: #666666; font-size: 28rpx; line-height: 40rpx; }
