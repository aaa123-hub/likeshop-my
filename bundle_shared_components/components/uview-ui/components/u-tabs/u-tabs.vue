				...
				<view class="u-scroll-box" :class="{'u-tabs-scorll-flex': !isScroll}" :style="{height: height + 'rpx'}">
					<view class="u-tab-item u-line-1" :id="'u-tab-item-' + index" v-for="(item, index) in list" :key="index" @tap="clickTab(index)"
					 :style="[tabItemStyle(index)]">
						<view :count="item[count] || item['count'] || 0" :offset="offset" size="mini"></view>
						{{ item[name] || item['name']}}
					</view>
...
<script>
	 ...
	 * @property {String} active-color 滑块和激活tab文字的颜色（默认#a0610d）
	 ...
	 * @example <view ref="tabs" :list="list" :is-scroll="false"></view>
	 */
	export default {
		name: "u-tabs",
		props: {
