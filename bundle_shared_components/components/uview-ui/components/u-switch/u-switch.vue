	...
	<view class="u-switch" :class="[value == true ? 'u-switch--on' : '', disabled ? 'u-switch--disabled' : '']" @tap="onClick"
	 :style="[switchStyle]">
		<view class="u-switch__node node-class" :style="{
			...
			<view :show="loading" class="u-switch__loading" :size="size * 0.6" :color="loadingColor" />
		</view>
	</view>
</template>
...
<script>
	 ...
	 * @property {String} active-color 打开时的背景色（默认#a0610d）
	 ...
	 * @example <view v-model="checked" active-color="red" inactive-color="#eee"></view>
	 */
	export default {
		name: "u-switch",
		props: {
